...
const AssetTypeOption = ({
  ...
  return (
    <div onClick={onClick}>
      {index !== 0 ? (
        <hr className="h-px mx-6 bg-gray-200 border-0 dark:bg-gray-700" />
      ) : null}
      <div className="w-full cursor-pointer my-1 hover:bg-dropdown-hover">
        <div
          ...
          <div className="flex inline">
            <Icon
              className="ml-5 w-8 rounded-full"
              name={assetType.icon as IconName}
            />
            <div className="p-2 pl-3">
              <div className="text-sm block text-white">
                <p className="text-white text-lg unselectable-text">
                  {assetType.ticker}
                </p>
              </div>
          ...
          <div className="text-white text-md font-red-hat-mono pr-7">
            {balance?.toString()}
          </div>
        </div>
      </div>
...
const AssetSelectModal = ({
  ...
  const filteredBlances = filteredAssetTypes.map((option) => {
    return (
      balances.find(
        (balance) => balance?.assetType.assetId === option.assetId
      ) || null
    );
  });
  ...
  return (
    <div className="w-96 bg-fourth -mx-6 -my-6 rounded-xl pt-4 pb-2">
      <div className="text-white unselectable-text text-lg pl-8 pt-1">
        Select a Token
      </div>
      <div
      ...
      <div className="mt-1 ml-2 overflow-y-auto h-64">
        {options.length ? (
          ...
          <div className="text-manta-gray text-sm text-center mt-4">
            No results found.
          </div>
        )}
        {}
...
const AssetTypeSelectButton = ({
  ...
  const disabled =
    txStatus?.isProcessing() || !senderAssetTypeOptions || !assetType;
  ...
  return (
    <div>
      <div
        ...
        <Icon className="w-3 h-3" name={'assetSelector'} />
      </div>
      <ModalWrapper>
        <AssetSelectModal
          hideModal={hideModal}
